...
We can plot samplef from our posterior at various levels of pi and sigma hat and compute how often the null is appropriately rejected.
...
We can see that the maximal power achieved is roughly 0.5, which is due to the fact that the demonstrated logOR considered here, -0.44, and corresponding standard error, 0.21, are so small, that the maximal posterior probability that they arose from the non-zero component is ~0.08 even with a pi_alternative of 0.99.
...
sigmas=seq(minsig,maxsig,by=inc)
plot(sigmas,powermat,xlab="PriorSigma", main=expression("Power vs", pi0) ,pch=0.1)
```
